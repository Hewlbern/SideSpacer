...
import stripe
stripe.api_key = "sk_test_s8t4a5sw1OoBFdKElK9lI8Mo"
...
def updateTransactions(request, subscription_id):
    ...
    return redirect(reverse('memberships:profile'))
...
def cancelSubscription(request):
    ...
    free_membership = Membership.objects.get(membership_type='Free Membership')
    user_membership = get_user_membership(request)
    user_membership.membership = free_membership
    user_membership.save()

    messages.info(request, "Successfully canceled the membership.")
    return redirect(reverse('memberships:select'))